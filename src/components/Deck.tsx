    ...
    <div className={`glass-panel rounded-2xl p-6 h-full flex flex-col relative overflow-hidden group border-t border-white/10 ${accentBorder}`}>
      ...
      <div className="flex-1 flex flex-col items-center justify-center text-center mb-4 relative z-10 min-h-[140px]">
        {track ? (
          ...
          <button
            onClick={onLoadTrack}
            aria-label={`Load Track into Deck ${isDeckA ? 'A' : 'B'}`}
            title={`Load Track into Deck ${isDeckA ? 'A' : 'B'}`}
            className={`w-full h-full border-2 border-dashed border-zinc-800 rounded-2xl flex flex-col items-center justify-center text-zinc-600 hover:border-zinc-600 hover:text-zinc-400 transition-all cursor-pointer group/empty bg-zinc-900/30 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset ${isDeckA ? 'focus-visible:ring-indigo-400' : 'focus-visible:ring-purple-400'}`}
          >
