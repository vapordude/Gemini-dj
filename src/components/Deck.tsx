    ...
    <div className={`glass-panel rounded-2xl p-6 h-full flex flex-col relative overflow-hidden group border-t border-white/10 ${accentBorder}`}>
      ...
      <div className="flex-1 flex flex-col items-center justify-center text-center mb-4 relative z-10 min-h-[140px]">
        {track ? (
          ...
          <button
            onClick={onLoadTrack}
            aria-label={`Load track on Deck ${id}`}
            title={`Load track on Deck ${id}`}
      ...
      <div className="mb-4 relative z-10">
        ...
        <div
          role="slider"
          tabIndex={0}
          aria-label={`Scrub track on Deck ${id}`}
          ...
          onKeyDown={e => {
            if (!duration) return;
            // Seek forward or backward 5 seconds on arrow keys
            if (e.key === 'ArrowRight') {
              e.preventDefault();
      ...
      <div className="grid grid-cols-4 gap-3 relative z-10 h-32">
        ...
        <div className="col-span-1 flex flex-col gap-2">
          ...
          <button
            ...
            aria-label={`Toggle 4-beat loop on Deck ${id}`}
            title={`Toggle 4-beat loop on Deck ${id}`}
            className={`flex-1 rounded-lg border flex flex-col items-center justify-center gap-1 transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400 ${
              isLooping
        ...
        <div className="col-span-2 flex flex-col gap-2">
          <div className="flex gap-2 h-1/2">
            <div className="flex-1 bg-zinc-900/50 rounded-lg border border-white/5 flex flex-col items-center justify-center p-1">
              <label htmlFor={`filter-deck-${id}`} className="text-[7px] font-bold text-zinc-500 uppercase mb-1">FILTER</label>
              <input
                id={`filter-deck-${id}`}
                type="range" min="20" max="22000" step="100" value={filterFreq}
                aria-label={`Filter Deck ${id}`}
                className="w-full h-1 bg-zinc-700 rounded-full appearance-none accent-yellow-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400 focus-visible:ring-offset-2 focus-visible:ring-offset-zinc-900"
                onChange={e => controls.setFilter(Number(e.target.value))}
              />
            </div>
            <div className="flex-1 bg-zinc-900/50 rounded-lg border border-white/5 flex flex-col items-center justify-center p-1">
              <label htmlFor={`echo-deck-${id}`} className="text-[7px] font-bold text-zinc-500 uppercase mb-1">ECHO</label>
              <input
                id={`echo-deck-${id}`}
                type="range" min="0" max="1" step="0.1" value={delayWet}
                aria-label={`Echo Deck ${id}`}
                className="w-full h-1 bg-zinc-700 rounded-full appearance-none accent-cyan-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400 focus-visible:ring-offset-2 focus-visible:ring-offset-zinc-900"
            ...
            {(['high', 'mid', 'low'] as const).map(band => (
              <div key={band} className="h-full flex flex-col items-center justify-center w-8 group/eq">
                <div className="h-8 w-1 bg-zinc-800 rounded-full relative">
                  <div
                    className="absolute w-3 h-2 bg-zinc-500 rounded-sm left-1/2 -translate-x-1/2 shadow-sm transition-all group-hover/eq:bg-white peer-focus-visible:ring-2 peer-focus-visible:ring-indigo-400 peer-focus-visible:ring-offset-2 peer-focus-visible:ring-offset-zinc-900"
                  ...
                  <input
                    ...
                    aria-label={`${band} EQ Deck ${id}`}
                    title={`${band} EQ Deck ${id}`}
