    ...
    <div className={`glass-panel rounded-2xl p-6 h-full flex flex-col relative overflow-hidden group border-t border-white/10 ${accentBorder}`}>
      ...
      <div className="mb-4 relative z-10">
        ...
        <div
          ...
          aria-label={`Track progress Deck ${id}`}
        ...
        <div className="col-span-2 flex flex-col gap-2">
          <div className="flex gap-2 h-1/2">
            <div className="flex-1 bg-zinc-900/50 rounded-lg border border-white/5 flex flex-col items-center justify-center p-1">
              <label className="text-[7px] font-bold text-zinc-500 uppercase mb-1" id={`filter-label-${id}`}>FILTER</label>
              <input
                type="range" min="20" max="22000" step="100" value={filterFreq}
                aria-labelledby={`filter-label-${id}`}
                title={`Filter Deck ${id}`}
            ...
            <div className="flex-1 bg-zinc-900/50 rounded-lg border border-white/5 flex flex-col items-center justify-center p-1">
              <label className="text-[7px] font-bold text-zinc-500 uppercase mb-1" id={`echo-label-${id}`}>ECHO</label>
              <input
                type="range" min="0" max="1" step="0.1" value={delayWet}
                aria-labelledby={`echo-label-${id}`}
                title={`Echo Deck ${id}`}
            ...
            {(['high', 'mid', 'low'] as const).map(band => (
              <div key={band} className="h-full flex flex-col items-center justify-center w-8 group/eq">
                <div className="h-8 w-1 bg-zinc-800 rounded-full relative">
                  ...
                  />
                  <div
                    className={`absolute w-3 h-2 bg-zinc-500 rounded-sm left-1/2 -translate-x-1/2 shadow-sm transition-all group-hover/eq:bg-white peer-focus-visible:ring-2 peer-focus-visible:bg-white ${isDeckA ? 'peer-focus-visible:ring-indigo-400' : 'peer-focus-visible:ring-purple-400'}`}
