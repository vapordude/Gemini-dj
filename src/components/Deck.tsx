        ...
        <div className="col-span-2 flex flex-col gap-2">
          <div className="flex gap-2 h-1/2">
            <div className="flex-1 bg-zinc-900/50 rounded-lg border border-white/5 flex flex-col items-center justify-center p-1">
              <label htmlFor={`filter-${id}`} className="text-[7px] font-bold text-zinc-500 uppercase mb-1 cursor-pointer">FILTER</label>
              <input
                id={`filter-${id}`}
                type="range" min="20" max="22000" step="100" value={filterFreq}
                title="Filter Frequency"
                className="w-full h-1 bg-zinc-700 rounded-full appearance-none accent-yellow-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400 cursor-pointer"
                onChange={e => controls.setFilter(Number(e.target.value))}
              />
            </div>
            <div className="flex-1 bg-zinc-900/50 rounded-lg border border-white/5 flex flex-col items-center justify-center p-1">
              <label htmlFor={`echo-${id}`} className="text-[7px] font-bold text-zinc-500 uppercase mb-1 cursor-pointer">ECHO</label>
              <input
                id={`echo-${id}`}
                type="range" min="0" max="1" step="0.1" value={delayWet}
                title="Echo Effect Amount"
                className="w-full h-1 bg-zinc-700 rounded-full appearance-none accent-cyan-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400 cursor-pointer"
            ...
            {(['high', 'mid', 'low'] as const).map(band => (
              <div key={band} className="h-full flex flex-col items-center justify-center w-8 group/eq">
                <div className="h-8 w-1 bg-zinc-800 rounded-full relative">
                  ...
                  />
                  <div
                    className="absolute w-3 h-2 bg-zinc-500 rounded-sm left-1/2 -translate-x-1/2 shadow-sm transition-all group-hover/eq:bg-white peer-focus-visible:ring-2 peer-focus-visible:ring-indigo-400"
                    style={{ bottom: `${((state[`eq${band.charAt(0).toUpperCase() + band.slice(1)}` as keyof DeckState] as number + 20) / 30) * 100}%` }}
                  />
                </div>
                <label htmlFor={`eq-${band}-${id}`} className="text-[6px] font-bold text-zinc-600 uppercase mt-1 cursor-pointer">{band.substring(0, 1)}</label>
              </div>
            ))}
          </div>
        </div>
