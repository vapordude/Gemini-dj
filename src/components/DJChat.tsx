        ...
        <div className="p-4 bg-zinc-900/50 border-t border-white/10">
          <div className="flex gap-2 mb-3">
            ...
            <button
              ...
              title={isTyping ? 'Generating response...' : (!input.trim() ? 'Message cannot be empty' : 'Send message')}
              className={`p-2.5 bg-indigo-600 rounded-xl text-white transition-all shadow-lg border border-indigo-400/50 ${
                isTyping
                  ? 'opacity-50 cursor-wait'
                  : !input.trim()
                  ? 'opacity-50 cursor-not-allowed'
                  : 'hover:bg-indigo-500 shadow-indigo-500/20'
              }`}
            >
              <Send size={18} />
            </button>
