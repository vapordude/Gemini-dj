        ...
        <div className="p-4 bg-zinc-900/50 border-t border-white/10">
          <div className="flex gap-2 mb-3">
            <input
              ...
              placeholder={isTyping ? "Generating response..." : "Enter command or request..."}
              disabled={isTyping}
              className="flex-1 bg-zinc-950/50 border border-white/10 rounded-xl px-4 py-2.5 text-xs text-white placeholder-zinc-600 focus:outline-none focus:border-indigo-500/50 focus:bg-zinc-900 transition-all font-mono shadow-inner disabled:opacity-50 disabled:cursor-wait"
            />
            <button
              onClick={handleSend}
              disabled={isTyping || !input.trim()}
              aria-label="Send message"
              title={isTyping ? "Waiting for AI..." : !input.trim() ? "Type a message to send" : "Send message"}
              className="p-2.5 bg-indigo-600 rounded-xl text-white hover:bg-indigo-500 transition-all shadow-lg shadow-indigo-500/20 border border-indigo-400/50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <Send size={18} />
            </button>
          </div>
