...
export function DJChat({ onCommand }: DJChatProps) {
  ...
  return (
    <>
      {/* Toggle Button */}
      <button
        aria-label="Toggle AI DJ Chat"
        title="Toggle AI DJ Chat"
        ...
        <div className="p-4 bg-zinc-900/50 border-t border-white/10">
          <div className="flex gap-2 mb-3">
            ...
            <button
              ...
              title={isTyping ? "AI is generating a response..." : (!input.trim() ? "Enter a message to send" : "Send message")}
              onClick={handleSend}
              disabled={isTyping || !input.trim()}
              className={`p-2.5 rounded-xl text-white transition-all shadow-lg border border-indigo-400/50 ${
                isTyping || !input.trim()
