...
## 2026-08-30 - Contextual Focus Rings on Repeated Components
**Learning:** When styling interactive elements across repeated components like 'Deck', applying context-aware focus rings (e.g., indigo for Deck A, purple for Deck B) alongside explicit 'focus-visible:outline-none focus-visible:ring-2' improves both keyboard accessibility and user context without breaking the glassmorphism UI style.
**Action:** Use conditional logic based on unique component identifiers (like 'id' prop) to apply distinct Tailwind focus-visible colors, and pair them with unique 'aria-label' and 'title' attributes containing the identifier (e.g., 'Play Deck A').
