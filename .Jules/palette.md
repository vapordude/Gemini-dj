...
## 2026-05-28 - Custom Sliders and Hidden Inputs Accessibility
**Learning:** Custom UI elements like waveform progress bars (built with `div` tags) and stylized faders/knobs (using hidden `input` ranges) fail keyboard accessibility without proper markup. Users cannot tell when these elements have focus, and screen readers lack context.
**Action:** Always add `role="slider"`, `tabIndex={0}`, and `aria-*` attributes (valuenow, valuemin, valuemax) to custom `div`-based sliders, along with explicit `onKeyDown` handlers for arrow keys. For stylized components with visually hidden `<input type="range">` elements, use Tailwind's `peer` utility on the input and `peer-focus-visible` on the stylized sibling to render focus rings. The input must precede the stylized sibling in the DOM. Ensure an `aria-label` is applied to the hidden input.
