...
## 2026-06-20 - Visually Hidden Custom Inputs and Keyboard Navigation
**Learning:** Custom UI elements like EQ sliders and crossfaders that visually hide the native `<input type="range">` element frequently lose visible keyboard focus, confusing keyboard/screen-reader users.
**Action:** When hiding inputs (e.g. `opacity-0`), always ensure the `<input>` element comes *before* its styled visual counterpart in the DOM. Apply the Tailwind `peer` class to the hidden `<input>`, and use `peer-focus-visible:ring-2 peer-focus-visible:ring-indigo-400 peer-focus-visible:outline-none` on the styled sibling to ensure keyboard focus states are clearly communicated. Furthermore, ensure the `<input>` retains `aria-label` and `title` attributes.
