...
## 2024-05-19 - Accessible Custom Sliders with Tailwind Peer
**Learning:** Custom UI sliders (where the native `<input type="range">` is visually hidden using `opacity-0`) lose default keyboard focus indicators, making them inaccessible to keyboard users.
**Action:** Always add descriptive `aria-label` and `title` attributes to the hidden native input. To restore focus visibility, use Tailwind's `peer` utility: apply `peer` and a higher `z-index` to the `<input>`, position it before the styled element in the DOM, and use `peer-focus-visible:ring-2` on the adjacent visible sibling to display focus rings.
