...
## 2025-02-27 - Focus Rings on Visually Hidden Sliders
**Learning:** When styling custom sliders by making the native `<input type="range">` visually hidden (`opacity-0`), keyboard focus states are lost. Adding standard focus classes to the wrapper or custom elements doesn't trigger when the native input receives focus.
**Action:** Use Tailwind's `peer` utility pattern. Apply `className="... peer"` to the visually hidden `<input>`, and add `peer-focus-visible:ring-2` to the adjacent custom visual element (like the slider thumb). This natively links the input's focus state to the visual presentation without needing JavaScript event listeners.
