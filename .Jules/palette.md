...
**Learning:** Custom UI interactive elements that act as buttons or sliders (e.g. waveform progress bars and dropzones) must be built with explicit ARIA roles (`role="slider"`), native elements (`<button>`), keyboard navigation event listeners (like `onKeyDown` to handle ArrowLeft/ArrowRight), and `tabIndex={0}` to be fully accessible.
**Action:** Always provide native keyboard equivalents, proper focus indicators (like `focus-visible:ring-2`), and ARIA attributes (such as `aria-valuenow`, `aria-valuemin`, `aria-valuemax`, and `aria-label`) to custom interactive elements.
