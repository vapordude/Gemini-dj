...
## 2025-02-19 - Semantic Buttons for Empty States in Repeated Components
**Learning:** Visually empty state areas that act as interactive actions (like 'Load Track' dropzones) should be built using semantic `<button>` elements rather than `div` elements to natively support keyboard focus management and screen readers. When these empty states exist in repeated components (like dual Decks), their focus rings should be contextually styled (e.g., indigo vs. purple) to visually distinguish them during keyboard navigation and improve user context.
**Action:** Always use `<button>` for interactive empty states and apply dynamic `focus-visible` styles that match the component's accent color to aid keyboard orientation.
