...
## 2025-03-08 - Keyboard Accessibility for Custom Sliders and Empty States
**Learning:** In React applications utilizing Tailwind CSS, custom interactive components like waveform progress bars constructed with `div`s lack native keyboard accessibility and semantics. Similarly, empty states designed as dropzones using `div` elements fail to participate in the tab sequence and cannot be triggered via keyboard.
**Action:** When creating custom sliders with `div`s, always apply `role="slider"`, `tabIndex={0}`, standard `aria-value*` attributes, and specifically implement native `onKeyDown` handlers (capturing `ArrowLeft`/`ArrowRight`) to enable keyboard seeking. For empty states with click handlers (e.g., "Load Track"), use semantic `<button>` tags instead of `div`s to automatically inherit focus management and key down events, coupling both with visual focus indicators.
