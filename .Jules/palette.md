...
**Learning:** Icon-only playback control buttons (Play, Pause, Sync, Loop) lack context for screen readers and lack keyboard interaction visibility, which degrades UX for a significant portion of users.
**Action:** Always apply dynamic `aria-label` and `title` attributes that update with state (e.g. Play vs Pause), and use `aria-pressed` for toggle states (Sync, Loop). Also ensure that `focus-visible` classes (like `focus-visible:ring-2`) are applied for keyboard accessibility.
