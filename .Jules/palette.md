...
## 2026-08-24 - [Slider Accessibility]
**Learning:** Custom UI elements acting as sliders (like waveform progress bars built with divs) require native HTML slider attributes (`role="slider"`, `tabIndex={0}`, `aria-valuenow`, `aria-valuemin`, `aria-valuemax`) along with explicit keyboard event handlers (like ArrowLeft/ArrowRight) and focus indicators for full accessibility.
**Action:** When implementing custom visual sliders or progress bars, always pair the click/drag logic with corresponding ARIA slider properties and native keyboard navigation.
