...
## 2025-02-23 - Focus Rings for Custom Sliders
**Learning:** Using native hidden <input type="range"> alongside custom slider visuals requires the hidden input to be placed BEFORE the visual element in the DOM to utilize Tailwind's `peer` and `peer-focus-visible` classes for focus rings.
**Action:** When building accessible custom controls, always ensure the hidden semantic element precedes its visual counterpart in the DOM structure, and explicitly set a higher z-index (e.g. `z-10`) on the hidden input so it remains interactive.
