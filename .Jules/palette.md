...
## 2024-05-19 - Semantic Buttons for Interactive Empty States
**Learning:** Empty states that behave as interactive actions (e.g. 'Load Track' dropzones in decks) are often mistakenly implemented as `div` elements, losing native keyboard focus management and screen reader support.
**Action:** When building visually empty states that are clickable, always use semantic `<button>` elements instead of `div`s. Pair this with explicit focus indicators (e.g., `focus-visible:ring-2`) and descriptive `aria-label`/`title` attributes that provide context (e.g., including the specific component ID like Deck A or Deck B).
