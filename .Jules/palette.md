...
## 2024-05-18 - Visually Hidden Range Inputs
**Learning:** For custom stylized inputs where the native input is visually hidden (`opacity-0`), screen readers still need `aria-label` and `title` to understand the input's purpose. Additionally, visual keyboard focus indicators are lost when the input is hidden.
**Action:** Use Tailwind's `peer` class on the hidden input, and conditionally style the visible adjacent element with `peer-focus-visible` to give the user keyboard focus feedback since the input itself is invisible. Also, always add `aria-label` and `title` to the hidden input.
