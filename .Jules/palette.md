...
## 2026-05-28 - Visually Hidden Inputs Focus Indicators
**Learning:** For custom stylized inputs where the native input element (like `<input type="range">`) is visually hidden (`opacity-0`), standard focus outlines are not visible to users navigating via keyboard, rendering the element inaccessible.
**Action:** Use Tailwind's `peer` class on the hidden input and conditionally style the visible adjacent styled sibling element with `peer-focus-visible` (e.g., `peer-focus-visible:ring-2`) to guarantee full keyboard focus visibility. Always ensure the native input has descriptive `aria-label` and `title` attributes.
