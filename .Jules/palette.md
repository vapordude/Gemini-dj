...
## 2024-05-19 - Semantic Buttons for Empty States and Contextual Focus Rings
**Learning:** Visually empty state areas that act as interactive actions (e.g., 'Load Track' dropzones) built as `div` elements are inaccessible to keyboard and screen readers. Additionally, when styling interactive elements across repeated components (like Decks A and B), using identical focus rings can confuse users about which component they are interacting with.
**Action:** Always use semantic `<button>` elements for empty states that trigger actions. Apply contextual focus rings (e.g., `focus-visible:ring-indigo-400` for Deck A and `purple-400` for Deck B) alongside explicit `focus-visible:outline-none focus-visible:ring-2` to visually distinguish keyboard navigation state and improve user context.
