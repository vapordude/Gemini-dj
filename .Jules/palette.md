...
## 2026-06-15 - Custom Interactive Control Accessibility
**Learning:** In audio/DJ applications, controls like custom EQ sliders (visually hidden range inputs layered over styled markers) and icon-only buttons (Play, Sync, Loop) require meticulous accessibility wiring. Simply adding tooltips isn't enough; the DOM order matters for CSS siblings, and custom "empty states" (like Load Track) must be native `<button>`s, not `<div>`s, to natively receive keyboard focus and enter-key activation.
**Action:** Always convert interactive placeholder `<div>`s to `<button>`s. For hidden native inputs driving custom UI, place the `<input className="peer">` *before* the visual element and use `peer-focus-visible` to render the focus ring on the custom element. Always explicitly link labels to inputs with `htmlFor` and `id`.
