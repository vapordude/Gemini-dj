...
## 2025-02-28 - Keyboard Accessible Waveform Seek
**Learning:** Custom interactive UI elements like track waveforms that act as sliders and are built from simple `div` elements require explicit `role="slider"`, `tabIndex={0}`, ARIA attributes (`aria-valuenow`, `aria-valuemin`, `aria-valuemax`), and explicit `onKeyDown` handlers for directional keyboard navigation (e.g. `ArrowLeft`/`ArrowRight`) to provide full keyboard and screen reader accessibility.
**Action:** Always verify that interactive elements modifying continuous values have appropriate ARIA roles, focus visibility, and keydown handlers when not using native `<input type="range">`.
