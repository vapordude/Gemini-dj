...
## 2026-09-08 - Deck Component Comprehensive Accessibility Pass
**Learning:** Custom interactive elements like custom volume sliders or stylized waveforms completely fail accessibility standards when built solely with `div` elements or visually hidden inputs without proper attributes. Users relying on screen readers or keyboard navigation are completely locked out of these critical playback controls.
**Action:** Always implement `role="slider"`, `tabIndex={0}`, `aria-valuenow`, `aria-valuemin`, and `aria-valuemax` on custom `div` sliders, coupled with `onKeyDown` handlers for ArrowLeft/ArrowRight navigation. For visually hidden inputs, apply `aria-label`, `title`, and tie their focus state to the visible custom handle using Tailwind's `peer` and `peer-focus-visible` utility classes.
