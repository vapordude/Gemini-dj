...
## 2024-05-19 - Semantic Buttons for Empty States
**Learning:** Empty states that are interactive (e.g. "Load Track" dropzones) should be implemented using semantic `<button>` elements rather than generic `<div>` elements. This provides native support for keyboard focus and screen readers.
**Action:** Always use `<button>` elements for interactive empty states and placeholders, accompanied by explicit `aria-label`, `title`, and context-aware keyboard focus indicators (e.g., `focus-visible:ring-2`).
