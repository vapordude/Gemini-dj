...
## 2025-03-09 - Accessible Focus States for Hidden Native Inputs
**Learning:** Custom UI components (like the physical fader cap in `Mixer.tsx` or stylized sliders) often visually hide the native `<input type="range">` by setting `opacity-0`. If the native input does not pass focus to the visible custom element, keyboard navigation users will not see any focus indication when tabbing through the interface, creating a severe accessibility barrier.
**Action:** Always use Tailwind's `peer` utility class on the hidden native input and apply `peer-focus-visible:ring-2` (or similar focus indicators) to the adjacent visible custom element. Ensure the native input precedes the custom element in the DOM for the `peer` class to work correctly, and include an `aria-label` or `title` on the hidden input for screen readers.
