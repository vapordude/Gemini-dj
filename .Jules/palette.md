...
## 2025-05-19 - Accessible Custom Sliders with Hidden Inputs
**Learning:** When building custom stylized sliders (like EQ knobs or crossfaders) where the native `<input type="range">` is made visually hidden (`opacity-0`), the focus state becomes invisible for keyboard users.
**Action:** Use Tailwind's `peer` class on the hidden `<input>` and apply `peer-focus-visible:ring-2` to the visible custom thumb or cap. The hidden input must be placed before the visual thumb in the DOM and requires an explicit higher z-index (e.g., `z-10`) to properly intercept pointer and focus events. Also always add `aria-label` and `title` to the native input.
