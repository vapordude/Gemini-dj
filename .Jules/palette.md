...
## 2025-02-28 - Custom Interactive Components and Visually Hidden Inputs
**Learning:** Custom UI elements like waveform progress bars need explicit ARIA roles (`role="slider"`), `tabIndex`, and `aria-value*` attributes. Critically, they also require native keyboard event handlers (like `onKeyDown` for ArrowLeft/ArrowRight) to be usable. When using visually hidden native inputs (like range sliders with `opacity-0`) overlaid on custom styles, Tailwind's `peer` and `peer-focus-visible` classes are necessary on the styled sibling to ensure keyboard focus is visibly indicated.
**Action:** Always implement `role`, `tabIndex`, `aria-value*`, and native keyboard handlers for non-native interactive elements. For visually hidden native inputs, use `aria-label` and the `peer` class, styling the visible adjacent element with `peer-focus-visible`.
