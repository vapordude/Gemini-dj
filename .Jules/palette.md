...
## 2026-08-08 - Accessible Custom EQ Knobs
**Learning:** Custom UI knobs/sliders built with visually hidden `input type="range"` elements and styled `div` thumbs are inaccessible to keyboard users if the `div` doesn't explicitly display focus state.
**Action:** Always add descriptive `aria-label` and `title` to the hidden input. Use Tailwind's `peer` class on the input and `peer-focus-visible` on the sibling thumb `div` to render a clear focus ring (e.g. `ring-2 ring-white`) when the input receives keyboard focus.
