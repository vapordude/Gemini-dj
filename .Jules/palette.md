...
## 2025-02-12 - Accessible Deck Controls
**Learning:** In repeated components like Decks, simply adding generic ARIA labels isn't enough; screen reader users need to know *which* deck a button controls. Additionally, empty state dropzones should be semantically marked up as buttons.
**Action:** Use the unique identifier of the repeated component (e.g., `id` 'A' or 'B') in `aria-label` strings (e.g., `Play Deck A`). Convert interactive empty states (like "Load Track") from `div` to `button` to natively support keyboard focus. Also, apply contextual focus rings (e.g., indigo for Deck A, purple for Deck B) to provide clear visual feedback during keyboard navigation.
