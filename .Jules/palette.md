...
## 2025-02-28 - Dynamic Contextual ARIA Labels for Repeated Components
**Learning:** When adding ARIA labels or tooltips to buttons in a repeated component like `Deck`, static labels (like "Play" or "Sync") are insufficient for screen readers because they cannot distinguish which deck's button is focused.
**Action:** Always use unique component identifiers (e.g., `id="A"`) to create dynamic, contextual labels (e.g., "Play Deck A") and match focus ring colors to the component's accent color (e.g., `focus-visible:ring-indigo-400` for Deck A) to provide clear context for both screen reader and keyboard users.
